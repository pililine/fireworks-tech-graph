...
Generate production-quality SVG technical diagrams exported as PNG via `rsvg-convert` (recommended default), `cairosvg`, or `puppeteer`.
...
9. **Export PNG**: Use `rsvg-convert` first. See **SVG → PNG Conversion** section below for full method comparison
...
|------|---------|----------------|-------|
| **`rsvg-convert` (recommended default)** | `brew install librsvg` / `apt install librsvg2-bin` | ✅ Good | Stable standalone binary; no Python runtime coupling |
| `cairosvg` | `brew install cairo pango gdk-pixbuf libffi` + `python -m pip install cairosvg` | ✅ Good | Good fallback; requires system cairo runtime and compatible Python env |
| `puppeteer` (headless Chrome) | `npm install puppeteer` | ✅✅ Best | Real browser engine; 100% fidelity but heavy (Node + Chromium) |

### Recommended Default: rsvg-convert
...
cairosvg input.svg -o output.png --output-width 2400

# Batch
for f in docs/00-core/*.svg; do
  cairosvg "$f" -o "${f%.svg}.png" --output-width 2400
done
```
...
- `cairosvg` depends on system cairo libraries; `pip install cairosvg` alone may be insufficient on some hosts
- `rsvg-convert` is usually the most stable export path for CI and mixed local Python environments
...
- On Apple Silicon, prefer Homebrew-managed toolchains under `/opt/homebrew`.
- Avoid mixing system Python, CommandLineTools Python, venv Python, and user-site packages in one export pipeline.
- If cairosvg fails with missing `libcairo`, install Homebrew cairo/librsvg dependencies first.
- If system Python still cannot load cairo-linked libraries, avoid global `DYLD_*` hacks; use `rsvg-convert` or a Homebrew Python venv.

### Temporary Fallbacks (Not for CI)

- `sips` / `qlmanage` can be used as temporary local rescue tools.
- Do not treat them as primary CI or release export paths.
- Expect occasional differences in scale, canvas size, fonts, or final rendering.

### Picking a Method

1. **Default** → `rsvg-convert` (stable CLI, no Python runtime coupling)
2. **If needed** → `cairosvg` (after system cairo libraries are installed)
3. **Browser-generated SVG or pixel-perfect required** → `puppeteer`
4. **Temporary local rescue only** → `sips` / `qlmanage`
